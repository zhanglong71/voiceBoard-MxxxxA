/**
 ...
 ===============================================================================
    ...
    [..]
        (#) 用RCC_APB2PeriphClockCmd(RCC_APB2Periph_USART1, ENABLE)使能USART1或
            使用RCC_APB1PeriphClockCmd(RCC_APB1Periph_USARTx, ENABLE)使能UART2、UART3、UART4。

        (#) 根据USART模式，使能GPIO时钟使用RCC_AHBPeriphClockCmd()函数（IO可以是TX、RX、CLK）。

        (#) 外设复用功能：
            (++)连接引脚到所需外设使用复用函数RCC_AHBPeriphClockCmd()。
            (++)通过GPIO_PuPd，GPIO_OType和GPIO_Speed选择上下拉，输出类型和输出速度。
            (++)调用GPIO_Init()函数。
        (#) 使用USART_Init()函数配置波特率，字长，停止位，奇偶校验，硬件流控和模式（接收/发送）。
        (#) 要使能同步模式，需要使用USART_ClockInit()函数使能时钟，并配置极性，相位和最后一个时钟脉冲位。
        (#) 如果需要使用中断，使用USART_ITConfig()使能NVIC和相应的中断。

        (#) 使用USART_Cmd()函数使能USART。
    [..]
            更多细节请参考多处理器，LIN，半双工，智能卡，IrDA章节。
@endverbatim
...
void USART_DeInit(USART_TypeDef* USARTx)
{
  ...
  if (USARTx == USART1)
  {
    RCC_APBPeriph2ResetCmd(RCC_APBPeriph2_USART1, ENABLE);
    RCC_APBPeriph2ResetCmd(RCC_APBPeriph2_USART1, DISABLE);
  }
  ...
  {
    RCC_APBPeriph1ResetCmd(RCC_APBPeriph1_UART3, ENABLE);
    RCC_APBPeriph1ResetCmd(RCC_APBPeriph1_UART3, DISABLE);
  }
  else
  {
    RCC_APBPeriph1ResetCmd(RCC_APBPeriph1_UART4, ENABLE);
    RCC_APBPeriph1ResetCmd(RCC_APBPeriph1_UART4, DISABLE);
  }
...
/**
  ...
  *   This parameter can be one of the following values:
  *      USART1, UART2, UART3, UART4.
  * @param  USART_DirectionMode: specifies the USART direction.
...
void USART_AutoBaudRateConfig(USART_TypeDef* USARTx, uint32_t USART_AutoBaudRate)
{
  ...
  USARTx->CR2 &= (uint32_t)~((uint32_t)USART_CR2_ABRMOD);
...
/**
  ...
  *   This parameter can be one of the following values:
  *      USART1, UART2, UART3, UART4.
  * @param  USART_DEDeassertionTime: specifies the time between the middle of the last
...
/**
  ...
  *            @arg USART_FLAG_NF:  Noise Error flag.
