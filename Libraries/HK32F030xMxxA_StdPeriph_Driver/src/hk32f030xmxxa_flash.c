...
/**
  ...
  * @param  Address: pecifies the address to be programmed.
  * @retval FLASH Status: The returned value can be: FLASH_ERROR_PG,
  *         FLASH_ERROR_WRP, FLASH_COMPLETE or FLASH_TIMEOUT.
  */
...
{
  uint16_t WRP0_Data = 0x00FF, WRP1_Data = 0x00FF, WRP2_Data = 0x00FF, WRP3_Data = 0x00FF;
  ...
  if(status == FLASH_COMPLETE)
  {
    ...
    if((status == FLASH_COMPLETE) && (WRP1_Data != 0x00FF))
    {
      OB->WRP1 = WRP1_Data;
    ...
    if((status == FLASH_COMPLETE) && (WRP2_Data != 0x00FF))
    {
      OB->WRP2 = WRP2_Data;
    ...
    if((status == FLASH_COMPLETE) && (WRP3_Data != 0x00FF))
    {
      OB->WRP3 = WRP3_Data;
