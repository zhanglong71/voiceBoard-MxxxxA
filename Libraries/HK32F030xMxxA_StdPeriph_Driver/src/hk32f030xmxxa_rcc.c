/**
  ******************************************************************************
  * @file    hk32f030xmxxa_rcc.c
  * @author  AE Team
  * @version V1.0.1/2022-05-16
  *
  * @log  增加RCC_AdjustHSICalibrationValue和 RCC_TrimHSIValue
  * @log  修改RCC_SYSCLKConfig
  * @log  修改 RCC_GetClocksFreq
  ******************************************************************************
  ===============================================================================
 ...
 ===============================================================================
    ...
    [..] Once the device started from reset, the user application has to:
         ...
         (#) Configure the clock source(s) for peripherals which clocks are not
             derived from the System clock (ADC, I2C, USART, UART2/3/4,and IWDG)
...
/** @defgroup RCC  RCC
  * @brief RCC driver modules
  ...
  [..]一旦设备重新启动，用户应用程序必须:
    ...
    (#)配置AHB和APB总线预分配器
    (#)启用外围设备的时钟
    (#)对于一些外设的时钟不是来自于System Clock的，需要配置时钟源(ADC, I2C, USART, UART2/3/4，和IWDG)。
   @endverbatim
  * @{
  */
 ...
 ===============================================================================
    [..] This section provides functions allowing to configure the external clocks,
         ...
         (#) HSI (high-speed internal), 32 MHz factory-trimmed RC used directly .
             The HSI clock can be used also to clock the USART, I2C , ADC , UART2/3/4
         (#) LSI (low-speed internal), 114 KHz low consumption RC used as IWDG clock source.
...
void RCC_DeInit(void)
{
  ...
  /* Reset USART1SW[1:0], I2CSW,UART2SW[1:0],UART3SW[1:0] and UART4SW[1:0] bits */
...
/**
  ...
  *         STARTUP_TIMEOUT in hk32f030xmxxa.h file.
...
@verbatim
 ...
 ===============================================================================
    ...
    [..] This section provide functions allowing to configure the System, AHB and
       ...
       (+@) The UART2/3/4 clock which is derived from HSI, system clock (SYSCLK), APB.

         (#) The maximum frequency of the SYSCLK, HCLK and PCLK is 32 MHz for HK32F030mxxa(48 MHz for HK32F0301mxxa ).
...
/**
  ...
  * @param  RCC_UART3CLK: defines the UART3 clock source. This clock is derived
  *         from the HSI or System clock or  PCLK.
  *          This parameter can be one of the following values:
  *             @arg RCC_UART3CLK_PCLK: UART3 clock = APB Clock (PCLK)
  *             @arg RCC_UART3CLK_SYSCLK: UART3 clock = System Clock
  *             @arg RCC_UART3CLK_HSI: UART3 clock = HSI Clock
  * @retval None
  */
void RCC_UART3CLKConfig(uint32_t RCC_UART3CLK)
{
  ...
  /* Set USARTxSW bits according to RCC_USARTCLK value */
  RCC->CFGR3 |= RCC_UART3CLK;
}

/**
  * @brief  Configures the UART4 clock (UART4CLK).
  * @param  RCC_UART4CLK: defines the UART4 clock source. This clock is derived
  *         from the HSI or System clock or  PCLK.
  *          This parameter can be one of the following values:
  *             @arg RCC_UART4CLK_PCLK: UART4 clock = APB Clock (PCLK)
  *             @arg RCC_UART4CLK_SYSCLK: UART4 clock = System Clock
  *             @arg RCC_UART4CLK_HSI: UART4 clock = HSI Clock
  * @retval None
  */
void RCC_UART4CLKConfig(uint32_t RCC_UART4CLK)
{
  /* Check the parameters */
  assert_param(IS_RCC_UART4CLK(RCC_UART4CLK));

  /* Clear USART1SW[1:0] bit */
  RCC->CFGR3 &= ~RCC_CFGR3_UART4SW;

  /* Set USARTxSW bits according to RCC_USARTCLK value */
  RCC->CFGR3 |= RCC_UART4CLK;
}
...
void RCC_GetClocksFreq(RCC_ClocksTypeDef* RCC_Clocks)
{
  ...
  temp = (RCC->CFGR3 & RCC_CFGR3_USART1SW);

  switch(temp)
  {
    case 0x00:      /* USART1CLK Clock is PCLK Osc. */
      RCC_Clocks->USART1CLK_Frequency = RCC_Clocks->PCLK_Frequency;
      break;

    case 0x01:      /* USART1CLK Clock is SYSCLK Osc. */
      RCC_Clocks->USART1CLK_Frequency = RCC_Clocks->SYSCLK_Frequency;
      break;

    case 0x03:      /* USART1CLK Clock is USARTHSIPRE Clock */
      tmp = RCC->CFGR4 & RCC_CFGR4_USARTHSIPRE;
      RCC_Clocks->USART1CLK_Frequency = (HSI_VALUE << 1) / (tmp + 2);;
      break;
  ...
  temp = (RCC->CFGR3 & RCC_CFGR3_UART3SW) >> RCC_CFGR3_UART3SW_Pos;

  switch(temp)
  {
    case 0x00:      /* UART3CLK Clock is PCLK Osc. */
      RCC_Clocks->UART3CLK_Frequency = RCC_Clocks->PCLK_Frequency;
      break;

    case 0x01:      /* UART3CLK Clock is SYSCLK Osc. */
      RCC_Clocks->UART3CLK_Frequency = RCC_Clocks->SYSCLK_Frequency;
      break;

    case 0x03:      /* I2C2 Clock is USARTHSIPRE Clock */
      tmp = RCC->CFGR4 & RCC_CFGR4_USARTHSIPRE;
      RCC_Clocks->UART3CLK_Frequency = (HSI_VALUE << 1) / (tmp + 2);;
      break;
  ...
  temp = (RCC->CFGR3 & RCC_CFGR3_UART4SW) >> RCC_CFGR3_UART4SW_Pos;
  ...
  switch(temp)
  {
    ...
    case 0x03:      /* I2C2 Clock is USARTHSIPRE Clock */
      tmp = RCC->CFGR4 & RCC_CFGR4_USARTHSIPRE;
      RCC_Clocks->UART4CLK_Frequency = (HSI_VALUE << 1) / (tmp + 2);;
      break;
...
/**
  ...
  *             @arg RCC_APBPeriph2_USART1: USART1 clock
